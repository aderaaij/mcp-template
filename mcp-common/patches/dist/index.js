...
import { stdioToStatefulStreamableHttp, DEFAULT_SESSION_IDLE_MS, DEFAULT_SESSION_MAX_AGE_MS, DEFAULT_MAX_SESSIONS, DEFAULT_SWEEP_INTERVAL_MS, } from './gateways/stdioToStatefulStreamableHttp.js';
async function main() {
    const argv = yargs(hideBin(process.argv))
        ...
        description: 'Transport for output. Default is "sse" when using --stdio and "stdio" when using --sse or --streamableHttp.',
    })
        .option('port', {
        type: 'number',
        default: 8000,
        ...
        type: 'boolean',
        default: false,
        description: 'Whether the server is stateful. Only supported for stdio→StreamableHttp.',
    })
        .option('sessionTimeout', {
        type: 'number',
        description: 'Session timeout in milliseconds. Only supported for stateful stdio→StreamableHttp. If not set, the session will only be deleted when client transport explicitly terminates the session.',
    })
        .option('sessionIdleMs', {
        type: 'number',
        description: 'Reap a stateful session after this many ms with no POST request. Unlike --sessionTimeout, SSE stream reconnects do not count as activity, so this bounds child processes even for clients that keep a stream open. Default 1800000 (30 min).',
        default: DEFAULT_SESSION_IDLE_MS,
    })
        .option('sessionMaxAgeMs', {
        type: 'number',
        description: 'Reap a stateful session this many ms after it was created, however active. Default 43200000 (12 h).',
        default: DEFAULT_SESSION_MAX_AGE_MS,
    })
        .option('maxSessions', {
        type: 'number',
        description: 'Cap on concurrent stateful sessions; the least-recently-used are reaped above this. Default 64.',
        default: DEFAULT_MAX_SESSIONS,
    })
        .option('sweepIntervalMs', {
        type: 'number',
        description: 'How often to check stateful sessions for reaping. Default 60000 (1 min).',
        default: DEFAULT_SWEEP_INTERVAL_MS,
    })
        .option('protocolVersion', {
        type: 'string',
        description: 'MCP protocol version to use for auto-initialization. Defaults to "2024-11-05" if not specified.',
        default: '2024-11-05',
    })
        .help()
        .parseSync();
    ...
    if (activeCount === 0) {
        logger.error('Error: You must specify one of --stdio, --sse, or --streamableHttp');
        process.exit(1);
    }
    else if (activeCount > 1) {
        logger.error('Error: Specify only one of --stdio, --sse, or --streamableHttp, not multiple');
        process.exit(1);
    }
            ...
            if (argv.outputTransport === 'sse') {
                ...
                if (stateful) {
                    ...
                    await stdioToStatefulStreamableHttp({
                        ...
                        sessionTimeout,
                        sessionIdleMs: argv.sessionIdleMs,
                        sessionMaxAgeMs: argv.sessionMaxAgeMs,
                        maxSessions: argv.maxSessions,
                        sweepIntervalMs: argv.sweepIntervalMs,
                    });
                }
                else {
